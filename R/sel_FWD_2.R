...
# #############################################################################
# # In this implementation the trimming units are the same for both grouping and no grouping models,
# # the union of trimmed units from both model

# redda_varsel_gr_fwd_union_trimming <- function(Xtrain,
#                                 cltrain,
#                                 emModels1 = c("E","V"),
...
#     }
#     
#     union_trimmed_units <- union(fit_grouping$train$obs_trimmed, worst_subset)
#     untrimmed_units <-
#       setdiff(1:fit_grouping$Ntrain, union_trimmed_units)
#     
#     xBIC <- TBIC_computation(
#       X_p = NULL,
#       X_c = Xtrain[, i],
#       cltrain = cltrain,
#       union_trimmed_units = union_trimmed_units,
...
#       silent = TRUE)
#       
#       # Get the union of the trimmed units for both models for computing the TBIC on the same set of obs
#       
#       union_trimmed_units <- union(fit_no_grouping$train$obs_trimmed, fit_grouping$train$obs_trimmed)
#       
...
#       }
#       
#       union_trimmed_units <- union(fit_grouping_current$train$obs_trimmed, worst_subset)
#       
#       oneBIC <- 2*sum(
#         dnorm(
#           x = S[-union_trimmed_units, ,drop=FALSE],
#           mean = fit_no_grouping$parameters$mean,
#           sd = sqrt(fit_no_grouping$parameters$variance$sigmasq),
#           log = TRUE
#         )
#       ) - 2*log(length(S[-union_trimmed_units, ,drop=FALSE]))
#       
...
#             silent = TRUE)
#             
#             union_trimmed_units <- union(fit_no_grouping$train$obs_trimmed, fit_grouping_current$train$obs_trimmed)
#             
#             cindepBIC <- TBIC_computation(
#               X_p = S[,i],
#               X_c = S[,-i, drop=FALSE],
